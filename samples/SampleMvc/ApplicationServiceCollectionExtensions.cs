...
namespace SampleMvc
{
    public static class ApplicationServiceCollectionExtensions
    {
        public static IServiceCollection AddApplicationDocumentStore(this IServiceCollection services, IConfigurationRoot configuration)
        {
            ...
            string host = configuration["Data:Store:Host"], 
                username = configuration["Data:Store:Username"],
                password = configuration["Data:Store:Password"],
                database = configuration["Data:Store:Database"];
            ...
            if (!String.IsNullOrWhiteSpace(connectionString) && connectionString.Contains("{{DATABASE_PASSWORD}}") && !String.IsNullOrWhiteSpace(password))
                connectionString = connectionString.Replace("{{DATABASE_HOST}}", host)
                    .Replace("{{DATABASE_USER}}", username)
